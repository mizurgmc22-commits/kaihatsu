...
- **マスターデータのコード化**: 資機材カテゴリ・マスタは JSON/TypeScript などコード化されたファイルとして Git 管理し、TypeORM のシード／マイグレーションで投入する。リポジトリの履歴で差分を追跡し、環境再構築時も同一データを再現できるようにする。
- **初期化スクリプトの整備**: `reset-equipment.ts`（または同等のシードスクリプト）でカテゴリ／資機材を投入し、`npm run typeorm migration:run` など定型コマンドで適用する運用を徹底する。
- **DB バイナリの扱い**: `backend/data/database.sqlite` 等のローカル DB ファイルは Git から除外し、コード化されたシード＋バックアップファイルで状態を移送する。本番は PostgreSQL 等のサーバ型 RDB を採用し、マイグレーションを通じてスキーマ・マスタを適用する。
- **バックアップ／監査**: マスタデータ更新時は Pull Request でレビューし、本番 DB には自動バックアップとロールフォワード／ロールバック手順を用意する。MCP/API 連携で他システムと共有する場合も、API レイヤー経由で配信しソース・オブ・トゥルースをコードに一本化する。
