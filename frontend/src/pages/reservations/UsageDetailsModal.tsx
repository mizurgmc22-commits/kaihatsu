import {
  Modal,
  ModalOverlay,
  ModalContent,
  ModalCloseButton,
  ModalBody,
  ModalFooter,
  Button,
  FormControl,
  FormLabel,
  Input,
  Textarea,
  VStack,
  HStack,
  Text,
  Box,
  Icon,
  useColorModeValue,
  useToast,
} from "@chakra-ui/react";
import { useForm } from "react-hook-form";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { FiFileText, FiMapPin, FiArrowLeft, FiCheck } from "react-icons/fi";
import { createReservation } from "../../api/reservation";
import type { AvailableEquipment, ReservationInput } from "../../types/reservation";
import type { ApplicantData } from "./ApplicantInfoModal";
import type { UsagePeriodData } from "./UsagePeriodModal";

export interface UsageDetailsData {
  purpose: string;
  location: string;
  notes: string;
}

interface Props {
  isOpen: boolean;
  onClose: () => void;
  selectedDate: string | null;
  onComplete: () => void;
  onBack: () => void;
  equipment: AvailableEquipment[];
  customEquipmentName: string | null;
  quantities: Record<string, number>;
  applicantData: ApplicantData | null;
  usagePeriodData: UsagePeriodData | null;
  initialData?: UsageDetailsData;
}

export default function UsageDetailsModal({
  isOpen,
  onClose,
  selectedDate,
  onComplete,
  onBack,
  equipment,
  customEquipmentName,
  quantities,
  applicantData,
  usagePeriodData,
  initialData,
}: Props) {
  const toast = useToast();
  const queryClient = useQueryClient();

  const gradientBg = useColorModeValue(
    "linear(to-br, green.500, teal.500)",
    "linear(to-br, green.600, teal.600)"
  );

  const {
    register,
    handleSubmit,
  } = useForm<UsageDetailsData>({
    defaultValues: initialData || {
      purpose: "",
      location: "",
      notes: "",
    },
  });

  const formatDate = (dateStr: string | null) => {
    if (!dateStr) return "";
    const d = new Date(dateStr);
    return d.toLocaleDateString("ja-JP", {
      year: "numeric",
      month: "long",
      day: "numeric",
      weekday: "short",
    });
  };

  // 予約作成
  const createMutation = useMutation({
    mutationFn: async (reservations: ReservationInput[]) => {
      for (const reservation of reservations) {
        await createReservation(reservation);
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["reservations"] });
      queryClient.invalidateQueries({ queryKey: ["availableEquipment"] });
      queryClient.invalidateQueries({ queryKey: ["calendarEvents"] });
      toast({
        title: "予約を登録しました",
        description:
          equipment.length > 1
            ? `${equipment.length}件の機器の予約が正常に作成されました。`
            : "予約が正常に作成されました。",
        status: "success",
        duration: 3000,
      });
      onComplete();
    },
    onError: (error: any) => {
      console.error("Reservation Error:", error);
      toast({
        title: "予約に失敗しました",
        description:
          error.response?.data?.message ||
          error.message ||
          "エラーが発生しました",
        status: "error",
        duration: 5000,
      });
    },
  });

  const onSubmit = (data: UsageDetailsData) => {
    if (!applicantData || !usagePeriodData) {
      toast({
        title: "エラー",
        description: "申請者情報または利用期間が入力されていません",
        status: "error",
        duration: 3000,
      });
      return;
    }

    const baseReservation = {
      department: applicantData.department,
      applicantName: applicantData.applicantName,
      contactInfo: applicantData.contactInfo,
      startTime: `${usagePeriodData.startDate}T${usagePeriodData.startTime}:00`,
      endTime: `${usagePeriodData.endDate}T${usagePeriodData.endTime}:00`,
      purpose: data.purpose || null,
      location: data.location || null,
      notes: data.notes || null,
    };

    let reservations: ReservationInput[];

    if (equipment.length > 0) {
      reservations = equipment.map((eq) => ({
        ...baseReservation,
        equipmentId: eq.id,
        quantity: quantities[eq.id] || 1,
      }));
    } else {
      reservations = [
        {
          ...baseReservation,
          customEquipmentName: customEquipmentName?.trim() || "",
          quantity: quantities["custom"] || 1,
        },
      ];
    }

    createMutation.mutate(reservations);
  };

  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      size="xl"
      motionPreset="slideInBottom"
    >
      <ModalOverlay bg="blackAlpha.600" backdropFilter="blur(4px)" />
      <ModalContent borderRadius="xl" overflow="hidden">
        {/* ヘッダー */}
        <Box bgGradient={gradientBg} color="white" py={6} px={6}>
          <HStack spacing={3} mb={2}>
            <Icon as={FiFileText} boxSize={6} />
            <Text fontSize="xl" fontWeight="bold">
              利用詳細を入力
            </Text>
          </HStack>
          <Text fontSize="sm" opacity={0.9}>
            {formatDate(selectedDate)} の予約 - ステップ 3/3
          </Text>
        </Box>

        <ModalCloseButton color="white" />

        <Box as="form" onSubmit={handleSubmit(onSubmit)}>
          <ModalBody py={6}>
            <VStack spacing={4} align="stretch">
              <FormControl>
                <FormLabel>利用目的</FormLabel>
                <Textarea
                  {...register("purpose")}
                  placeholder="例: 新人研修でのCPR訓練"
                  rows={2}
                  borderRadius="lg"
                />
              </FormControl>

              <FormControl>
                <FormLabel>
                  <HStack spacing={2}>
                    <Icon as={FiMapPin} />
                    <Text>利用場所</Text>
                  </HStack>
                </FormLabel>
                <Input
                  {...register("location")}
                  placeholder="例: 3階 研修室A"
                  borderRadius="lg"
                />
              </FormControl>

              <FormControl>
                <FormLabel>備考</FormLabel>
                <Textarea
                  {...register("notes")}
                  placeholder="その他連絡事項があれば記入してください"
                  rows={2}
                  borderRadius="lg"
                />
              </FormControl>
            </VStack>
          </ModalBody>

          <ModalFooter
            borderTopWidth="1px"
            bg={useColorModeValue("gray.50", "gray.800")}
          >
            <HStack spacing={4} w="full" justify="space-between">
              <Button
                variant="ghost"
                leftIcon={<FiArrowLeft />}
                onClick={onBack}
              >
                利用期間に戻る
              </Button>

              <HStack spacing={3}>
                <Button variant="ghost" onClick={onClose}>
                  キャンセル
                </Button>
                <Button
                  type="submit"
                  rightIcon={<FiCheck />}
                  isLoading={createMutation.isPending}
                  bgGradient={gradientBg}
                  color="white"
                  _hover={{
                    bgGradient: "linear(to-br, green.600, teal.600)",
                    transform: "translateY(-1px)",
                    shadow: "lg",
                  }}
                  _active={{
                    transform: "translateY(0)",
                  }}
                >
                  予約を確定
                </Button>
              </HStack>
            </HStack>
          </ModalFooter>
        </Box>
      </ModalContent>
    </Modal>
  );
}
