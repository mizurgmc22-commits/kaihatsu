...
const CATEGORY_DISPLAY_ORDER = ['蘇生講習資機材', 'トレーニング資機材', '機械類', '消耗品', 'その他'];
...
export default function AdminGantt() {
  ...
  const { data: events, isLoading } = useQuery({
    ...
    queryFn: () => getCalendarEvents(monthStart.toISOString(), monthEnd.toISOString())
  });

  const { data: equipmentData, isLoading: isEquipmentLoading } = useQuery({
    queryKey: ['equipment', 'gantt-view'],
    queryFn: () => getEquipmentList({ page: 1, limit: 1000 })
  });
  ...
  const assignLanes = (list: CalendarEvent[]): { events: TimelineEvent[]; laneCount: number } => {
    if (!list.length) {
  ...
  const categoryGroups = useMemo<EquipmentCategoryGroup[]>(() => {
    ...
    return Array.from(groups.entries())
      ...
      .map(([categoryName, equipments]) => ({
        categoryName,
        equipments: equipments.sort((a, b) => a.equipmentName.localeCompare(b.equipmentName, 'ja'))
      }));
  }, [equipmentData, eventsByEquipment]);

  const flatTimelines = useMemo(() => categoryGroups.flatMap((group) => group.equipments), [categoryGroups]);

  const equipmentColorMap = useMemo(() => {
    const map = new Map<string, string>();
    flatTimelines.forEach((timeline) => {
      const categoryColor = CATEGORY_COLOR_MAP[timeline.categoryName] ?? DEFAULT_CATEGORY_COLOR;
  ...
  const getBarPosition = (event: CalendarEvent) => {
    ...
    const left = ((boundedStart.getTime() - monthStart.getTime()) / totalDuration) * 100;
    const width = ((boundedEnd.getTime() - boundedStart.getTime()) / totalDuration) * 100;
    ...
    <Box>
      <Flex justify="space-between" align="center" mb={6}>
        ...
        <HStack spacing={2}>
          ...
          />
          <Box textAlign="center" px={4} py={2} bg="white" borderRadius="md" shadow="sm">
